...
(function bootstrapCampaignRuntime(root) {
  ...
  function gatePanel() {
    ...
    if (gate.status === 'owner_preview') return `<div class="dnd-runtime-callout success"><strong>Private preview enabled</strong><span>Release remains prohibited. No updater, tag, deployment, or public Discord delivery is authorized.</span></div>`;
    return `<form data-runtime-form="enable" class="dnd-runtime-stack"><div class="dnd-runtime-callout warning"><strong>Private development build</strong><span>This runtime is disabled until the Owner enters the exact phrase. Release remains prohibited.</span></div><label>Confirmation phrase<input name="confirmation" autocomplete="off" placeholder="ENABLE D&D RUNTIME PREVIEW" required></label><button class="button primary" type="submit">Enable Private Preview</button></form>`;
  }

  function profilePanel() {
    const profile = state.payload?.profiles?.[0] || {};
    return `<form data-runtime-form="profile" class="dnd-runtime-stack"><div class="panel-heading"><div><span class="eyebrow">Campaign control model</span><h4>Play Profile</h4></div><span class="tag">${esc(profile.mode || 'not configured')}</span></div><div class="form-grid"><label>Mode<select name="mode"><option value="solo_ai_dm">Solo · AI DM</option><option value="group_ai_dm">Group · AI DM</option><option value="human_dm">Human DM</option><option value="human_dm_with_ai">Human DM + Veyra</option><option value="hybrid">Hybrid handoff</option></select></label><label>Pace<select name="pace"><option value="live">Live</option><option value="asynchronous">Asynchronous</option><option value="mixed">Mixed</option></select></label><label>Automation<select name="automationLevel"><option value="suggestions_only">Suggestions only</option><option value="narration_and_npcs">Narration and NPCs</option><option value="automatic_checks">Automatic checks</option><option value="automatic_combat">Automatic combat</option><option value="full_ai_dm">Full AI DM profile</option></select></label><label class="toggle-row"><span>Apply validated narrative events</span><input name="applyNarrativeEvents" type="checkbox" ${profile.automation?.applyNarrativeEvents ? 'checked' : ''}></label></div><div class="dnd-runtime-callout"><strong>Hard limits</strong><span>Mechanical events and automatic Discord publication remain disabled regardless of the selected profile.</span></div><button class="button primary" type="submit">Save Private Profile</button></form>`;
  }
  ...
  function runPanel() {
    ...
    if (!run) return `<form data-runtime-form="run" class="dnd-runtime-stack"><div class="panel-heading"><div><span class="eyebrow">Authoritative state</span><h4>Campaign Run</h4></div></div><label>Starting world time<input name="worldTime" value="Day 1"></label><button class="button primary" type="submit">Start Private Campaign Run</button></form>`;
  ...
  async function submit(event) {
    ...
    try {
      ...
      await refresh(true); notify('Private D&D runtime updated.');
  ...
  async function click(event) {
    ...
    try {
      ...
      if (button.dataset.runtimeAction === 'restore') await invoke('dnd:campaign-runtime-checkpoint-restore', { campaignId: selectedCampaignId(), checkpointId: button.dataset.checkpointId, confirmed: root.confirm('Restore this checkpoint and replace current campaign runtime state?') });
      await refresh(true);
    } catch (error) { notify(error.message || String(error)); }
    finally { setBusy(false); }
  }
