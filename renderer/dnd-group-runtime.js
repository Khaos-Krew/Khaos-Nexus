...
(function bootstrapGroupRuntime(root) {
  ...
  function render(){
    ...
    mount.innerHTML=`<article class="panel dnd-group-runtime"><div class="panel-heading"><div><span class="eyebrow">Production D&D runtime</span><h3>Group Play Coordination</h3><p>Live or asynchronous action collection, absence handling, voting, and reviewed delivery drafts. No Discord message is sent automatically.</p></div><button class="button" data-group-action="refresh" ${state.busy?'disabled':''}>Refresh</button></div><div class="dnd-group-grid"><section>${sessionPanel()}</section><section>${roundPanel()}</section><section>${decisionPanel()}</section><section>${deliveryPanel()}</section></div><div class="dnd-runtime-callout success"><strong>Production safeguards active</strong><span>Group coordination is release-authorized while Discord delivery remains review-only and never automatic.</span></div></article>`;
  }
  async function submit(event){
    ...
    try{setBusy(true);
      ...
      await refresh(true);notify('Group state updated.');
