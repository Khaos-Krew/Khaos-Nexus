...
(function bootstrapSoloCombat(root) {
  ...
  function render() {
    ...
    mount.innerHTML = `<article class="panel dnd-solo-combat"><div class="panel-heading"><div><span class="eyebrow">Private development slice</span><h3>Solo Play & Combat Engine</h3><p>Deterministic campaign actions remain local and unreleased. Veyra cannot directly change mechanical state.</p></div><button class="button" data-solo-action="refresh" ${state.busy ? 'disabled' : ''}>Refresh</button></div><div class="dnd-solo-grid"><section>${soloPanel()}</section><section>${combatPanel()}</section><section>${memoryPanel()}</section><section><div class="dnd-runtime-callout warning"><strong>Release prohibited</strong><span>No tag, updater, deployment, automatic Discord post, or public build is authorized.</span></div><p class="muted">This slice supports solo setup, inspectable memory, initiative, action economy, attacks, damage, concentration checks, death saves, and checkpointed combat completion.</p></section></div></article>`;
  }
  ...
  async function submit(event) {
    ...
    try {
      ...
      await refresh(true); notify('Private solo/combat state updated.');
