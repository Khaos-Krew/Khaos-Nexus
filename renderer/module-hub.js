...
(() => {
  ...
  function applyStaticCopy() {
    ...
    if (banner) {
      ...
      if (heading) heading.textContent = 'Local Nexus command network';
      if (description) description.textContent = 'Your runtime, game servers, modules, logs, backups and protected settings are managed from this PC.';
    }
  }
  ...
  function summaryCard(label, value, detail, tone = '') {
    return `<article class="module-summary-card ${tone}"><span>${escapeHtml(label)}</span><strong>${escapeHtml(value)}</strong><small>${escapeHtml(detail)}</small></article>`;
  }

  function renderSummary() {
    const summary = ui.payload?.summary || {};
    const target = $('moduleSummary');
    if (!target) return;
    target.innerHTML = [
  ...
  function renderFilters() {
    ...
    const rail = $('moduleWorkspaceRail');
    ...
    rail.innerHTML = ['all', ...workspaces].map((workspace) => `<button class="module-workspace-chip ${ui.workspace === workspace ? 'active' : ''}" data-module-workspace="${escapeHtml(workspace)}"><span>${workspace === 'all' ? 'All Workspaces' : escapeHtml(workspace)}</span><small>${workspace === 'all' ? ui.payload.catalog.length : ui.payload.catalog.filter((module) => module.workspace === workspace).length}</small></button>`).join('');
  }
  ...
  function renderGrid() {
    const target = $('nexusModuleGrid');
    if (!target) return;
    const modules = filteredCatalog();
    if (!modules.length) {
      target.innerHTML = '<article class="panel module-empty"><strong>No modules match these filters.</strong><span>Clear the search or choose another workspace.</span></article>';
      return;
    }
  ...
  function renderDetail() {
    const target = $('nexusModuleDetail');
    if (!target) return;
    const module = (ui.payload?.catalog || []).find((item) => item.id === ui.selectedId) || filteredCatalog()[0];
    if (!module) {
      target.innerHTML = '<div class="module-detail-empty"><span>◇</span><strong>Select a Nexus module</strong><p>Open a card to review its functions, dependencies and migration checklist.</p></div>';
      return;
    }
  ...
  function openWorkspace(moduleId) {
    ...
    const button = document.querySelector(`[data-view="${CSS.escape(module.launchView)}"], [data-view-link="${CSS.escape(module.launchView)}"]`);
    if (!button) { notify('The target workspace is still initializing.'); return; }
    button.click();
  }

  async function refresh() {
    const payload = await invoke('modules:get');
    if (!payload || !Array.isArray(payload.catalog)) return;
    ui.payload = payload;
    if (!ui.selectedId || !payload.catalog.some((module) => module.id === ui.selectedId)) ui.selectedId = payload.catalog[0]?.id || null;
    render();
  }

  async function updateSelected(patch) {
    if (!ui.selectedId) return;
    ui.payload = await invoke('modules:update', { id: ui.selectedId, patch });
    render();
  }

  function bindShell() {
    $('moduleRefreshButton')?.addEventListener('click', refresh);
    $('moduleExportButton')?.addEventListener('click', async () => {
      const result = await invoke('modules:export-roadmap');
      if (!result?.canceled) notify('Module roadmap exported.');
    });
    ...
    $('moduleWorkspaceRail')?.addEventListener('click', (event) => {
      const chip = event.target.closest('[data-module-workspace]');
      if (!chip) return;
      ui.workspace = chip.dataset.moduleWorkspace;
      renderFilters(); renderGrid(); renderDetail();
    });
    $('nexusModuleGrid')?.addEventListener('click', (event) => {
      ...
      const details = event.target.closest('[data-module-details], [data-module-id]');
      const id = details?.dataset.moduleDetails || details?.dataset.moduleId;
      if (id) { ui.selectedId = id; renderGrid(); renderDetail(); }
    });
    $('nexusModuleDetail')?.addEventListener('click', async (event) => {
      const open = event.target.closest('[data-module-open]');
      if (open) { openWorkspace(open.dataset.moduleOpen); return; }
      if (event.target.closest('#moduleToggleButton')) {
        const module = ui.payload.catalog.find((item) => item.id === ui.selectedId);
        await updateSelected({ enabled: !module.state?.enabled });
      }
      if (event.target.closest('#moduleSaveNotesButton')) await updateSelected({ notes: $('moduleNotesInput')?.value || '' });
    });
    $('nexusModuleDetail')?.addEventListener('change', async (event) => {
      const step = event.target.closest('[data-module-step]');
      if (!step) return;
      ui.payload = await invoke('modules:mark-step', { id: ui.selectedId, stepId: step.dataset.moduleStep, completed: step.checked });
      render();
    });
  }
  ...
  async function initialize() {
    ...
    document.addEventListener('click', (event) => {
      if (event.target.closest('[data-view="modules"], [data-view-link="modules"]')) setTimeout(() => refresh().catch(() => {}), 0);
    });
    await refresh();
  }
