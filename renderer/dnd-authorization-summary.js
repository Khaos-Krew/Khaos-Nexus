  ...
  async function renderAuthorizationSummary() {
    ...
    try {
      ...
      const signature = JSON.stringify({
        apps: apps.map((app) => [app.id, app.name, app.enabled, app.modules, app.hasToken]),
      ...
      panel.innerHTML = `
        ...
        <div class="callout">Tokens are never shown here. Each bot lists only its configured campaign, guild, and least-privilege D&amp;D scopes.</div>
        <div class="dnd-list">
          ${apps.length ? apps.map((app) => {
            ...
            return `<div class="dnd-binding-card">
              <div>
                ...
                <span>${app.enabled === false ? 'Disabled' : 'Enabled'} · ${app.hasToken ? 'Protected token present' : 'Token missing'} · ${(app.modules || []).includes('dnd-workspace') ? 'D&D enabled' : 'D&D disabled'}</span>
