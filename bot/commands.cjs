...
function createCommands({ isModuleEnabled = () => true } = {}) {
  const serverOption = (builder, required = true) => builder
    .addStringOption((option) => option
      .setName('server')
      .setDescription('Configured Palworld server name')
      .setRequired(required)
      .setAutocomplete(true));

  const playerOption = (builder) => builder
    .addStringOption((option) => option.setName('player').setDescription('Palworld player name or user ID').setRequired(true));

  const commands = [
    new SlashCommandBuilder().setName('ping').setDescription('Check whether Nexus Sentinel is responding.'),
    new SlashCommandBuilder().setName('health').setDescription('Show Nexus Sentinel runtime health.'),
    serverOption(new SlashCommandBuilder().setName('status').setDescription('Show Palworld server status and health.')),
    serverOption(new SlashCommandBuilder().setName('players').setDescription('List connected Palworld players without exposing IP addresses.')),
    ...
    serverOption(new SlashCommandBuilder().setName('saveworld').setDescription('Save a Palworld server world.')),
    serverOption(new SlashCommandBuilder().setName('broadcast').setDescription('Broadcast a message to a Palworld server.'))
      .addStringOption((option) => option.setName('message').setDescription('Message to broadcast').setRequired(true).setMaxLength(500)),
    playerOption(serverOption(new SlashCommandBuilder().setName('kick').setDescription('Kick a player from a Palworld server.')))
      .addStringOption((option) => option.setName('message').setDescription('Optional reason shown to the player').setMaxLength(300)),
    playerOption(serverOption(new SlashCommandBuilder().setName('ban').setDescription('Ban a player from a Palworld server.')))
      .addStringOption((option) => option.setName('message').setDescription('Optional reason shown to the player').setMaxLength(300)),
    playerOption(serverOption(new SlashCommandBuilder().setName('unban').setDescription('Unban a Palworld player account.'))),
    ...
    serverOption(new SlashCommandBuilder().setName('forcestop').setDescription('Immediately stop a Palworld server.'))
      .addBooleanOption((option) => option.setName('confirm').setDescription('Confirm emergency force-stop').setRequired(true)),
    serverOption(new SlashCommandBuilder().setName('rcon').setDescription('Run an advanced Owner console command on a Palworld RCON connection.'))
      .addStringOption((option) => option.setName('command').setDescription('Raw Palworld console command').setRequired(true).setMaxLength(1000)),
    new SlashCommandBuilder().setName('listservers').setDescription('List enabled Palworld servers and connection types.'),
    new SlashCommandBuilder().setName('managerrestart').setDescription('Ask the desktop manager to restart Nexus Sentinel.')
  ];
