...
The exact current rebuild head `10bc45d4…` passed **Nexus Rebuild CI #663**. The 17 commits since README validation commit `bc20fad0…` add persistent hosted-server management, safe public-registry privacy controls, provider/runtime status, Nitrado-backed Palworld status, and focused persistence/privacy/provider regression coverage. This establishes exact-head automated validation only; live hosted-server/Nitrado owner acceptance, broader Discord acceptance, and public/stable release status remain separate.
