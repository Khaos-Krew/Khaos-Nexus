...
**Sentinal owns the self-role migration path.** PR #329 unified generic self-role/color menus; #330 expanded bounded discovery; #331 added safe migration of older emoji reaction-role messages; and #335 added direct adoption of the old bot-authored Nexus button menus. The live hardening path then added exact target diagnostics/false-positive filtering (#339), deterministic mappings for renamed legacy panels while refusing ambiguous duplicates and leaving generic `LFG` unresolved (#341), preservation of custom Discord Name Color swatches (#342), restart reconstruction (#343), and strict module category/channel visibility reconciliation (#344).

**PRs #345 and #346 are merged and exact-head green.** #345 moved generated Name Color swatches from guild custom emoji to Sentinal application emoji after live testing hit Discord's guild custom-emoji limit. #346 fixed the follow-up gray-swatch defect by explicitly preserving the live Discord role color/hex through alias augmentation and generating/reusing deterministic hex-keyed application swatches. Live acceptance still needs to confirm both Name Color pages show distinct blocks matching their real role colors and that switching remains globally exclusive across both pages.

**PRs #347 and #349 harden persistent Sentinal panels across deployments.** #347 makes managed module hubs and persistent live feeds deployment-idempotent: when state pointers are stale or missing, Sentinal discovers its own existing canonical panel, edits/reuses it, removes redundant Sentinal-owned copies, and persists the surviving message ID instead of blindly sending another message. #349 adds one bounded first-reconciliation status line per feed action so Railway acceptance can distinguish reused/updated/created/unavailable/error, recovery source, surviving message ID, and duplicate cleanup without making normal recurring polls noisy. Both exact heads passed Nexus Rebuild CI; live repeated-deployment acceptance remains pending.
...
The latest exact merged implementation head with completed rebuild validation is **PR #349 head `0d300731bf3d8c5c55fb8651cd5e4657416aa4e2`**: **Nexus Rebuild CI run #322 passed**, and PR #349 merged into `rebuild/nexus-0.1` as `48db224b4a8937a1d8ea248cd9cc599a97c5a046`. PR #347 head `c3bd6e652f930c443b99ff474eea049e3a692da0` passed run #320 before merging. This is automated evidence only; final owner acceptance, live-provider correctness, real hosted provider synchronization/persistence, explicit hosted-pairing validation, live role assignment/restart behavior, module channel visibility isolation, persistent hub/feed idempotency, safety-report acceptance, updater apply/rollback success, Thora owner validation, and public/stable release remain separate gates.

The current [`Discord + Nexus Setup Acceptance`](docs/DISCORD_NEXUS_SETUP_ACCEPTANCE.md) checkpoint records useful hosted and Owner interaction evidence. Role migration is materially safer and restart reconstruction is implemented, but real-guild acceptance is still pending—especially actual role assignment after restart, remaining generic `LFG`, duplicate-role safety, final legacy-control retirement, end-to-end Name Color switching with correct application-owned color swatches, verification that module access roles isolate each game category correctly without breaking staff visibility, and repeated Railway deployments proving one canonical module hub/feed panel survives without duplicate recreation.
...
- **Next — Provider-backed game services:** providers, authenticated hosted sync, persistent hosted-provider storage, guarded read-only validation, event feeds, capability-driven help, interactive ARK taming, and Warframe Archon Hunt reads are merged; exercise Palworld first, then other supported providers, and validate companion/feed surfaces in real Discord usage.
- **Then — Sentinal operational acceptance:** self-role/reaction/button migration, deterministic aliases, restart reconstruction, strict module access policy, application-owned/live-role-hex Name Color fixes (#345/#346), and deployment-idempotent module hubs/live feeds with acceptance telemetry (#347/#349) are merged; complete live restart/assignment, correct swatch rendering, module visibility, feed/hub idempotency, Name Color exclusivity/hierarchy, reporting, moderation, pairing, provider, and remaining Discord + Nexus Setup Acceptance gates before declaring the operational phase accepted.
