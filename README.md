...
Release hardening has also advanced materially. PR #363 makes dependency installs deterministic with a lockfile and `npm ci`; PR #362 promotes the exact Windows artifact already validated by Nexus Rebuild CI rather than rebuilding it during release publication; PR #364 adds isolated Windows CI smoke coverage for the real NSIS clean install, packaged startup/backend health, staged update apply, post-update confirmation, and payload hash match; PR #365 keeps those reproducibility checks valid inside the production runtime image without weakening repository CI.

Recent exact-head CI evidence includes PR #364 head `ce62a70715cc1aae59cb6c477aba242ba21c8e65` on Nexus Rebuild CI #372, PR #365 head `cd2e90c250e76697e2809cd296ee55a1ba9b67d7` on run #371, and PR #363 head `3ec4a49699dc6961b7f0c9e822e9c1eb61448cbf` on run #367; all were green before merge. PR #362 is merged as the exact-artifact promotion/provenance slice. No release is published or authorized merely by these changes.

This is automated and hosted-runtime evidence only. Real member interaction, provider credentials, desktop pairing, report lifecycle, moderation, owner-machine updater behavior, and public/stable release remain separate gates.

The current [`Discord + Nexus Setup Acceptance`](docs/DISCORD_NEXUS_SETUP_ACCEPTANCE.md) checkpoint remains active. Role authority plus the expanded game-module layout, access-policy reconciliation, category ordering, hub idempotency, feed recovery, and staff Name Color display safety now have live evidence. Fresh pairing, the corrected desktop confirmation scan, live Repair Nexus where needed, real provider sync/validation, and desktop/hosted restart persistence still require owner interaction.
...
- **Then — Sentinal operational acceptance:** role authority is live-accepted at 100%, and module layout/idempotency plus staff-compatible Name Color display safety now have live evidence; broader acceptance still requires normal-member button/command use, report lifecycle, moderation, pairing, provider flows, Setup Acceptance, and restart behavior.
- **Release hardening:** deterministic installs, exact validated-artifact promotion/provenance, staged updating, and Windows clean-install/staged-upgrade smoke validation are merged and green; next validate the same flow on an actual owner installation, including startup confirmation and rollback, while keeping public/stable publication a separate explicit decision.
