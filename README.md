...
**Sentinal owns the self-role migration path.** PR #329 unified generic self-role/color menus; #330 expanded bounded discovery; #331 added safe migration of older emoji reaction-role messages; and #335 added direct adoption of the old bot-authored Nexus button menus. The live hardening path has now advanced further: **#339 adds exact legacy-button target diagnostics and excludes false positives such as the Community Rules report control and owner-testing poll; #341 deterministically maps five renamed legacy panels to existing roles while refusing ambiguous duplicates and leaving generic `LFG` unresolved; and #342 preserves the original custom Discord color-swatch emoji during migration while retaining global color exclusivity and safe hierarchy behavior.**

Live restart acceptance exposed one remaining blocker: **PR #343 is open because current migrated self-role panels can lose their runtime option/role mappings after a Sentinal restart, leaving buttons without a reconstructed runtime menu.** #343 also proposes deterministic recovery/creation of visual-only color swatches when an already-rewritten panel no longer contains the old custom emoji reference. The PR is not part of the accepted baseline until it merges and passes validation.
...
The latest exact merged implementation head with completed rebuild validation is **PR #342 head `0702ca5c78faa156022ee45859ec0faf35fde5cb`**: **Nexus Rebuild CI run #296 passed**, and PR #342 merged into `rebuild/nexus-0.1`. PR #341 head `a2e20744a0dc320fb660a074563379332f663081` passed run #294, and PR #339 head `4762480c0d894ecf7eae3bcfa3f506d7d530f701` passed run #291. **PR #343 head `12e11736015172b40868b337d0e95da3bc26c63f` is open/unmerged and CI run #299 is still in progress.** This is automated evidence only; final owner acceptance, live-provider correctness, real hosted provider synchronization/persistence, explicit hosted-pairing validation, live self-role migration/restart recovery, safety-report acceptance, updater apply/rollback success, Thora owner validation, and public/stable release remain separate gates.

The current [`Discord + Nexus Setup Acceptance`](docs/DISCORD_NEXUS_SETUP_ACCEPTANCE.md) checkpoint records useful hosted and Owner interaction evidence. Role migration is now materially safer and more deterministic, but real-guild acceptance is still pending—especially restart recovery, the remaining generic `LFG` decision, duplicate-role safety, final legacy-control retirement, and end-to-end Name Color behavior.
...
- **Then — Sentinal operational acceptance:** unified self-role/color menus, reaction-role migration, direct legacy button adoption, exact target diagnostics, deterministic renamed-panel aliases, and custom Name Color swatches are merged, but **restart reconstruction remains an active blocker in PR #343**; complete the remaining Discord + Nexus Setup Acceptance gates before declaring old controls retired.
