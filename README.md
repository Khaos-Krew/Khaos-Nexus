...
**PR #317 aligns hosted provider persistence with Sentinal by making provider state honor `NEXUS_DATA_DIR`, and adds the dedicated [`Discord + Nexus Setup Acceptance`](docs/DISCORD_NEXUS_SETUP_ACCEPTANCE.md) checkpoint.** The checkpoint separates verified CI/hosted runtime evidence from the Owner interactions that still have to be exercised.
...
The latest exact merged implementation head with completed rebuild validation is **PR #317 head `f5ce0835e846aa50b2557f5470bb9a927851e8e4`**: **Nexus Rebuild CI run #227 passed**. PR #317 then merged into `rebuild/nexus-0.1` as `134aae1dc104d48bca4a487950dbaae3a58fa316`. This is automated evidence only; owner acceptance, real-guild behavior, live-provider correctness, real hosted provider synchronization/persistence, hosted-pairing owner validation, Setup Center owner usability, moderation behavior on the intended guild, Thora owner validation and public/stable release remain separate gates.

The current Discord + Nexus Setup Acceptance checkpoint also records successful hosted evidence from the PR #316 Railway deployment: health passed, Sentinal logged into Discord, persistent module feeds started, required friendly commands registered without replacing unrelated commands, and the module access menu reconciled with 10 roles, 1 message and 0 warnings. Remaining Owner gates include desktop `/nexus-pair`, Setup Center hosted scan, live repair where needed, at least one real provider sync/read-only validation, and restart-persistence checks.
...
- **Then — Sentinal operational acceptance:** setup/repair, persistent panels, friendly commands, event feeds, recurring/durable rank-SKU discovery, module-access reconciliation, warning diagnostics, protected-role fallback, administrator moderation, hosted pairing and hosted provider routes are merged; complete the Owner interaction gates in the Discord + Nexus Setup Acceptance checkpoint on the real guild and intended Railway Sentinal instance.
