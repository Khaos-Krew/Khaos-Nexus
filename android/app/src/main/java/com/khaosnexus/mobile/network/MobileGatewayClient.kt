...
class MobileGatewayClient(private val secureStore: SecureStore) {
    ...
    fun modules(session: StoredSession): List<ModuleData> = get(session, "/v1/modules").getJSONArray("modules").objects().map { value ->
        ModuleData(
            ...
            name = value.optString("name", "Function"),
            category = value.optString("category"),
            stage = value.optString("stage"),
            availability = value.optString("availability"),
            enabled = value.optBoolean("effectiveEnabled"),
            reason = value.optString("reason"),
            progress = value.optInt("progress"),
            workspace = value.optString("workspace"),
            description = value.optString("description"),
            requiredRole = value.optString("requiredRole", "viewer"),
            features = value.optJSONArray("features").strings(),
            statusLabel = value.optString("statusLabel")
        )
    }
