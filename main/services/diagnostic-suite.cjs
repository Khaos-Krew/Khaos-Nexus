...
class DiagnosticSuite {
  ...
  startSession(detail = {}) {
    ...
    this.session = { format: 'khaos-nexus-diagnostic-session', formatVersion: 1, id: crypto.randomUUID(), startedAt: iso(this.now()), appVersion: this.appVersion, installMode: this.mode, pid: this.pid, detail: redactObject(detail) };
    atomicJsonWrite(this.activeSessionPath, this.session);
    this.breadcrumb('session-started', { sessionId: this.session.id, installMode: this.mode });
  ...
  createReport(trigger = {}, context = {}) {
    ...
    const report = {
      ...
      application: { name: 'Khaos Nexus', version: this.appVersion, packaged: this.isPackaged, installMode: this.mode, executablePath: redactText(this.executablePath), resourcesPath: redactText(this.resourcesPath), dataDirectory: redactText(this.dataDirectory) },
  ...
  publicStatus() {
    ...
    return { diagnosticsDirectory: this.diagnosticsDirectory, installMode: this.mode, appVersion: this.appVersion, session: clone(this.session), previousSessionUnclean: this.hadUncleanPreviousSession(), settings: this.publicSettings(), latest: latest ? { reportId: latest.reportId, createdAt: latest.createdAt, trigger: latest.trigger, summary: latest.summary, checks: latest.checks, filePath: latest.filePath } : null, outboxCount: listFiles(this.outboxDirectory, (name) => name.endsWith('.json'), 1000).length };
  }
