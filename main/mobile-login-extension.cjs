...
const { enrichMobileFunctions } = require('../shared/mobile-functions.cjs');
...
function patchConfigStore() {
  ...
  if (!Original || Original.__khaosMobileLoginPatchedV2) return;
  ...
  Object.defineProperty(MobileLoginConfigStore, '__khaosMobileLoginPatchedV2', { value: true });
...
function patchGatewayRoute() {
  ...
  if (!prototype || prototype.__khaosMobileLoginRoutePatchedV2) return;
  ...
  Object.defineProperty(prototype, '__khaosMobileLoginRoutePatchedV2', { value: true });
}

function patchFunctionCatalog() {
  const target = require('./services/mobile-gateway-service.cjs');
  const prototype = target.MobileGatewayService?.prototype;
  if (!prototype || prototype.__khaosMobileFunctionsPatchedV1) return;
  const original = prototype.modules;
  prototype.modules = function mobileFunctions() {
    return enrichMobileFunctions(original.call(this), catalog());
  };
  Object.defineProperty(prototype, '__khaosMobileFunctionsPatchedV1', { value: true });
}
...
function patchBrowserLoader() {
  ...
  if (!prototype || prototype.__khaosMobileLoginUiPatchedV2) return;
  ...
  Object.defineProperty(prototype, '__khaosMobileLoginUiPatchedV2', { value: true });
}
...
function install() {
  ...
  patchConfigStore();
  patchGatewayRoute();
  patchFunctionCatalog();
  patchBrowserLoader();
  electron.app.whenReady().then(registerIpc);
}

module.exports = { install, patchConfigStore, patchGatewayRoute, patchFunctionCatalog, LOGIN_RATE_LIMIT };
