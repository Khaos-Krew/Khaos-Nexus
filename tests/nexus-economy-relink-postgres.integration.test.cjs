...
test('real Postgres relink preserves migrated wallet, proves ownership, rejects conflicts and survives restart with spending off', { skip: !process.env.NEXUS_TEST_POSTGRES_URL }, async () => {
  ...
  try {
    ...
    assert.equal(tokens.filter((result) => result.duplicate).length, 1);
    assert.equal(await runtime.walletCore.balance(discordUserId, 'DINO_CACHE_TOKENS'), 1);
    assert.equal(await runtime.worker.balance(discordUserId), 73);
    await assert.rejects(runtime.walletCore.spend({ ...tokenRequest, amount: 2 }));
    assert.equal((await runtime.walletCore.spend({ ...tokenRequest, amount: 2, orderId: 'cache_insufficient' })).reason, 'insufficient-funds');
    ...
    const purchase = { discordUserId, eosId, itemId: 'metal', idempotencyKey: 'shop_once' };
    assert.equal((await shop.createBuyOrder(purchase)).duplicate, false);
    assert.equal((await shop.createBuyOrder(purchase)).duplicate, true);
    assert.equal(await runtime.worker.balance(discordUserId), 33);
    assert.equal(await runtime.walletCore.balance(discordUserId, 'DINO_CACHE_TOKENS'), 1);
    await assert.rejects(shop.createBuyOrder({ ...purchase, bundles: 2 }));
    assert.equal(Number((await runtime.pool.query(`SELECT count(*) FROM "${schema}".nexus_economy_purchase_outbox`)).rows[0].count), 1);
  } finally {
