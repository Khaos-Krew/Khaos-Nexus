...
const { APEX_TRIBUTE_SELLS, sellEntry: apexSellEntry } = require('../src/sentinel/arkshop-nexus-launch-v9-apex-tribute-sell-startup.cjs');
...
test('launch readiness requires the complete static ARK shop baseline', () => {
  ...
  const SellItems = {
    ...
    ...Object.fromEntries(Object.entries(APEX_TRIBUTE_SELLS).map(([id, spec]) => [id, apexSellEntry(spec)]))
  };
  ...
  assert.equal(ready.counts.requiredApexTributeSells, 13);
});
...
test('launch readiness reports missing economy pieces', () => {
  ...
  assert.ok(result.missing.some((entry) => entry.startsWith('apex-tribute-sell:')));
});
