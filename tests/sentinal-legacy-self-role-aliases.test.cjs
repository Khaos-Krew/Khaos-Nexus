...
test('rules buttons and ordinary testing polls are excluded before role parsing', () => {
  ...
  assert.equal(shouldInspectLegacyRoleMessage({
    author: { bot: true },
    embeds: [{ title: 'KHAOS NEXUS • COMMUNITY RULES', footer: { text: 'Khaos Nexus • Community Rules' } }],
    components: [{ type: 1, components: [{ type: 2, label: 'Open Private Report', custom_id: 'nexus:safety:report' }] }]
  }), false);
  assert.equal(shouldInspectLegacyRoleMessage({
    author: { bot: true },
