...
test('defaults to Nexus economy authority and canonical Discord storefronts', () => {
  ...
  assert.equal(policy.storefronts.dinoCache, 'dino-cache');
});
...
test('legacy maintenance is opt-in and limited to compatibility mode', () => {
  assert.equal(resolveEconomyAuthorityPolicy({}).legacyArkShop.maintenanceEnabled, false);
  assert.equal(resolveEconomyAuthorityPolicy({ NEXUS_ARKSHOP_COMPAT_MAINTENANCE: 'true' }).legacyArkShop.maintenanceEnabled, false);
