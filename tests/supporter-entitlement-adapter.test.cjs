...
function member(id, memberRoles = [], editable = true) {
  return {
    id: String(id),
    user: { id: String(id), bot: false },
    roles: { cache: new Map(memberRoles.map((roleId) => [String(roleId), role(roleId, editable)])) }
  };
}
...
function fakeGuild({ memberRoles = [], editable = true } = {}) {
  ...
  const actions = [];
  const target = member('u1', memberRoles, editable);
  target.roles.add = async (item) => { actions.push(['add', String(item.id)]); };
  target.roles.remove = async (item) => { actions.push(['remove', String(item.id)]); };
  return {
    actions,
...
test('configured SKU and paid-role lists exclude free and legacy ranks', () => {
  assert.deepEqual(configuredSkuIds(config).sort(), ['11111', '22222', '33333', '44444']);
  assert.deepEqual(configuredPaidRoleIds(config).sort(), ['20000', '30000', '40000', '50000']);
});
...
test('stale paid-role discovery finds members with paid roles without treating Shadow Recruit or Origin Founder as paid', () => {
  const members = new Map([
    ['u1', member('u1', ['10000'])],
    ['u2', member('u2', ['90000'])],
    ['u3', member('u3', ['30000'])],
    ['u4', member('u4', ['50000', '90000'])]
  ]);
  assert.deepEqual(stalePaidRoleMemberIds(members, config).sort(), ['u3', 'u4']);
});
...
test('Server Shop role authority bypasses Premium App entitlement reconciliation', async () => {
  const serverShopConfig = { discord: { rankRoles: config.discord.rankRoles, rankSkus: {} } };
  const result = await reconcileSupporterEntitlements({}, {}, serverShopConfig);
  assert.equal(result.ok, true);
