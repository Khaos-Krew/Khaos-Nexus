...
test('Pokémon GO automatically becomes a Games access role when enabled', () => {
...
test('admin moduleEnabled overrides config so role menu follows live module controls', () => {
  ...
  assert.equal(definitions.some((item) => item.moduleId === 'warframe'), true);
});

test('disabled modules are omitted without invalidating deterministic custom IDs', () => {
...
test('role menu payload stays within Discord button row limits', () => {
...
test('button parser rejects unrelated and unknown IDs', () => {
  assert.equal(parseAccessButton('pogo:raid:rsvp'), null);
  assert.equal(parseAccessButton(`${ACCESS_BUTTON_PREFIX}not-a-module`), null);
});

test('rules cleanup removes website-labelled URL lines but preserves other links', () => {
  const input = ['**Server Rules**', '1. Be respectful.', 'Website: https://example.invalid', 'Discord help: https://discord.com/safety'].join('\n');
...
test('rules cleanup removes only website link buttons or exact configured website URL', () => {
  const rows = [{
    type: 1,
    components: [
...
test('rules cleanup returns no edit when there is no website link', () => {
  const message = {
