...
test('ActionStore projection strips metadata not covered by the canonical digest', () => {
  const record = buildRecord();
  const contaminated = { ...record, payload: { ...record.payload, command: 'untrusted', providerSecret: 'must-not-forward', nested: { unsafe: true } } };
  const prepared = createNexusEconomyPurchaseActionRequest().prepare(contaminated);
  assert.equal(prepared.ok, true);
  assert.deepEqual(prepared.actionRequest.request.payload, record.payload);
  assert.equal(JSON.stringify(prepared).includes('must-not-forward'), false);
  assert.equal(prepared.submitPermitted, false);
  assert.equal(prepared.executionPermitted, false);
});
