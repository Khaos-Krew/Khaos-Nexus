...
test('mobile login stores a salted verifier and never returns the password', () => {
  const record = createMobileLoginRecord({ username: 'Owner.Admin', password: 'correct-horse-battery' }, new Date('2026-08-17T19:00:00Z'));
...
test('mobile login verification is case-insensitive for username and exact for password', () => {
  const record = createMobileLoginRecord({ username: 'NexusOwner', password: 'A-Strong-Passphrase-26' });
  assert.equal(verifyMobileLogin(record, 'nexusowner', 'A-Strong-Passphrase-26'), true);
  assert.equal(verifyMobileLogin(record, 'NEXUSOWNER', 'A-Strong-Passphrase-26'), true);
  assert.equal(verifyMobileLogin(record, 'NexusOwner', 'wrong-password'), false);
  assert.equal(verifyMobileLogin(record, 'someone-else', 'A-Strong-Passphrase-26'), false);
});

test('mobile login rejects weak or malformed credentials', () => {
  assert.throws(() => createMobileLoginRecord({ username: 'ok-user', password: 'short' }), /at least 10/);
