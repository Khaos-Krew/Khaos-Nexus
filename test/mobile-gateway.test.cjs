...
test('P-256 device pairing stores hashed credentials and public state excludes secrets', () => {
  const key = ecPair();
  const details = publicKeyDetails(key.publicKeyPem);
  assert.equal(details.keyAlgorithm, 'EC-P256');
  ...
  const payload = publicMobileGatewayState({ enabled: true, devices: [issued.device] }, null, { transportReady: true, certificateFingerprint: 'AA:BB' });
  ...
  assert.equal(payload.transportReady, true);
});

test('pairing claim secret is hashed and request consumes the code', () => {
  const key = ecPair();
  const now = new Date('2026-07-23T18:00:00.000Z');
  const session = createPairingSession({ now, requestedRole: 'viewer' });
  ...
  assert.equal(verifyClaimSecret(created.request, created.claimSecret), true);
  assert.equal(verifyClaimSecret(created.request, `${created.claimSecret}bad`), false);
  assert.equal(created.consumedSession.pendingRequestId, created.request.id);
  assert.equal(created.request.address, '192.168.1.5');
});

test('signed mobile requests verify P-256 signatures, timestamps, and exact paths', () => {
  const pair = ecPair();
  const issued = issueDeviceCredential({ name: 'Phone', role: 'viewer', publicKeyPem: pair.publicKeyPem });
  const timestamp = '1785360000000';
  ...
  const signature = crypto.sign('sha256', Buffer.from(canonical), pair.privateKey).toString('base64url');
  assert.equal(verifyMobileRequestSignature(issued.device, { ...input, signature }, Number(timestamp)).ok, true);
  assert.equal(verifyMobileRequestSignature(issued.device, { ...input, path: '/v1/modules', signature }, Number(timestamp)).ok, false);
  assert.equal(verifyMobileRequestSignature(issued.device, { ...input, signature }, Number(timestamp) + 10 * 60 * 1000).code, 'TIMESTAMP_INVALID');
});

test('revocation and role boundaries remain immediate', () => {
  const key = ecPair();
  const issued = issueDeviceCredential({ name: 'Phone', role: 'operator', publicKeyPem: key.publicKeyPem });
