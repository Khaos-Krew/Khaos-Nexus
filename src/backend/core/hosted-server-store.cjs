...
const PROVIDER_TYPES = new Set(['none', 'palworld-rest', 'oncehuman-basic', 'custom']);
...
function normalizeHost(value) {
  return safeText(value, 253).toLowerCase();
}
...
function gameLabel(moduleId) {
  return moduleId === 'palworld' ? 'Palworld' : moduleId === 'oncehuman' ? 'Once Human' : moduleId;
}

function publicServer(server = {}) {
  return {
    id: String(server.id || ''),
    moduleId: String(server.moduleId || ''),
    game: gameLabel(server.moduleId),
    name: String(server.name || gameLabel(server.moduleId) || 'Server'),
    description: String(server.description || ''),
    joinInfo: server.public === false ? '' : String(server.joinInfo || ''),
    public: server.public !== false,
    providerType: String(server.providerType || 'none'),
    providerConfigured: String(server.providerType || 'none') !== 'none',
    providerConnected: Boolean(server.providerConnected),
    trackingState: server.providerConnected ? 'active' : 'configured',
...
function privateServer(server = {}) {
  return {
    ...
    adminPort: server.adminPort ?? null,
    credentialEnv: String(server.credentialEnv || '')
  };
}
...
class HostedServerStore {
  constructor(options = {}) {
    ...
    this.store = new JsonStore(filePath, { version: 1, servers: [] });
  ...
  get(id, { includePrivate = false } = {}) {
    ...
    return includePrivate ? privateServer(server) : publicServer(server);
  }
  ...
  add(input = {}) {
    ...
    if (servers.some((item) => item.moduleId === moduleId && normalizeHost(item.host) === host && Number(item.port) === port)) {
      throw new Error('That hosted server is already registered.');
    }
    const providerType = safeText(input.providerType || (moduleId === 'palworld' ? 'palworld-rest' : 'oncehuman-basic'), 40).toLowerCase();
    if (!PROVIDER_TYPES.has(providerType)) throw new Error('Unsupported provider type.');
    const timestamp = this.now();
    const server = {
      id: `SRV-${crypto.randomUUID().slice(0, 8).toUpperCase()}`,
      moduleId,
      name: safeText(input.name, 80) || gameLabel(moduleId),
      host,
      port,
      queryPort: normalizePort(input.queryPort, null),
      adminPort: normalizePort(input.adminPort, null),
      description: safeText(input.description, 300),
      joinInfo: safeText(input.joinInfo, 200),
    ...
    this.store.update((draft) => {
      draft.servers = Array.isArray(draft.servers) ? draft.servers : [];
  ...
  remove(id) {
    let removed = false;
    this.store.update((draft) => {
      const before = Array.isArray(draft.servers) ? draft.servers : [];
