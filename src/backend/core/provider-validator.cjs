...
class ProviderValidator {
  constructor({ runtime, now = () => Date.now(), probes = DEFAULT_PROBES } = {}) {
    if (!runtime || typeof runtime.invoke !== 'function' || typeof runtime.manifests !== 'function') {
      throw new Error('ProviderValidator requires a Nexus Backend runtime.');
    }
  ...
  async validateOne(moduleId) {
    ...
    const result = await this.runtime.invoke(moduleId, actionId, {}, {
      role: 'viewer',
      actorId: 'provider-validator',
      confirmed: false
    });
  ...
  invoke(_moduleId, actionId, payload = {}) {
    if (actionId !== 'validate-providers') throw new Error(`Unknown provider validation action: ${actionId}`);
    return this.validate(payload.moduleId || payload.module || '');
  }
}
