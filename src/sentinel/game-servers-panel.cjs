...
const GAME_SERVERS_PANEL_MARKER = 'Nexus Sentinal • Managed Game Servers • v3';
...
function trackingGlyph(server={}){const state=normalizedTrackingState(server);if(state==='online')return'🟢';if(['maintenance','starting','restarting','stopping','updating'].includes(state))return'🟡';if(state==='offline')return'🔴';if(state==='manual')return'🔵';return'🟡';}
function trackingLabel(server={}){const state=normalizedTrackingState(server);if(state==='online')return'Online';if(['maintenance','starting','restarting','stopping','updating'].includes(state))return'Maintenance';if(state==='offline')return'Offline';if(state==='manual')return'Registered • manual status';if(state==='not-configured')return'Registered • connection needs configuration';if(state==='registered')return'Registered • live telemetry unavailable';return server.providerConfigured===true?'Registered • telemetry pending':'Registered • live telemetry unavailable';}
function cleanPublicText(value,max=240){return String(value || '').replace(/[\r\n]+/g,' ').replace(/\s+/g,' ').trim().slice(0,max);}
function renderServerLine(server={}){
  const name=cleanPublicText(server.name || 'Server',80)||'Server';const badge=server.ownershipType==='community-approved'?'🌐 Nexus Approved':'🛡️ Khaos Nexus Official';const lines=[`${trackingGlyph(server)} **${name}**`,`${badge} • ${trackingLabel(server)}`];
...
function renderPrivateServerLine(server={}){const name=cleanPublicText(server.name || 'Private Server',80)||'Private Server';const game=cleanPublicText(server.game || server.moduleId || 'Game',80);const lines=[`${trackingGlyph(server)} **${name}**`,`${game} • ${trackingLabel(server)}`];const description=cleanPublicText(server.description,180);if(description)lines.push(description);return lines.join('\n');}
...
function renderGameServersPanel(snapshot={}){
  ...
  fields.push({name:'Community Server Program',value:'Want your server listed? Use **/server apply**. Community servers are reviewed for safety, transparency, and monetization policy before they can appear here. Approved servers can be automatically hidden if a reliable health check shows an extended outage.',inline:false});
  return{embeds:[{title:GAME_SERVERS_PANEL_TITLE,description:'Official Khaos Nexus servers and staff-approved community servers. Community approval is a listing review, not a guarantee or endorsement of every action by a third-party server. Network management ports, passwords, tokens, credentials, and staff-only notes are never displayed here.',color:servers.length||privateGroups.length?0x2ecc71:0x5865f2,fields:fields.slice(0,25),footer:{text:GAME_SERVERS_PANEL_MARKER}}],allowedMentions:{parse:[]}};
}
