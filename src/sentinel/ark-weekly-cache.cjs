...
function applyCurrentPrice(rotation) {
  return { ...rotation, cache:{ ...rotation.cache, price:WEEKLY_PRICE } };
}
function generateRotation(now, secret, previous = []) {
  ...
  return { id:String(startsAt), startsAt, endsAt:startsAt+WEEK, cache:{ price:WEEKLY_PRICE, cooldownMinutes:5, cooldownHours:1/12,
