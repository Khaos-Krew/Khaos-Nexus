...
function hostedServerCommand() {
  return new SlashCommandBuilder()
    .setName(SERVER_COMMAND)
    .setDescription('Manage Khaos Nexus hosted game servers')
    .addSubcommand((sub) => sub
      .setName('add')
      .setDescription('Register a hosted game server')
      .addStringOption((opt) => opt.setName('game').setDescription('Game').setRequired(true)
        .addChoices({ name: 'Palworld', value: 'palworld' }, { name: 'Once Human', value: 'oncehuman' }))
    ...
    .addSubcommand((sub) => sub
      .setName('edit')
      .setDescription('Edit a registered hosted server')
      ...
      .addStringOption((opt) => opt.setName('credential_env').setDescription('Environment variable name containing the credential').setRequired(false).setMaxLength(80))
      .addBooleanOption((opt) => opt.setName('public').setDescription('Show this server in #game-servers').setRequired(false)))
    .addSubcommand((sub) => sub
      .setName('remove')
      .setDescription('Remove a registered hosted server')
      .addStringOption((opt) => opt.setName('id').setDescription('Server ID from /server list').setRequired(true))
    ...
    .addSubcommand((sub) => sub.setName('refresh').setDescription('Refresh the public #game-servers registry'));
}
...
function addInput(interaction) {
  return {
    ...
    queryPort: optionValue(interaction, 'query_port'),
...
function editInput(interaction) {
  const mapping = {
    name: 'name', host: 'host', port: 'port', query_port: 'queryPort', admin_port: 'adminPort',
    description: 'description', join_info: 'joinInfo', credential_env: 'credentialEnv', public: 'public'
  };
  const input = {};
...
function privateServerList(servers = []) {
  ...
  return servers.map((server) => {
    const privateEndpoint = `${server.host || 'unknown'}${server.port ? `:${server.port}` : ''}`;
    const extra = [server.queryPort ? `query ${server.queryPort}` : '', server.adminPort ? `admin ${server.adminPort}` : '', server.credentialEnv ? `credential env ${server.credentialEnv}` : ''].filter(Boolean).join(' • ');
    return `**${server.id} — ${server.name}**\n${server.game} • ${privateEndpoint} • ${server.public === false ? 'private listing' : 'public listing'}${extra ? `\n${extra}` : ''}`;
  }).join('\n\n').slice(0, 3800);
}
...
async function handleHostedServerCommand(interaction, context = {}) {
  ...
  if (!(await context.isManager(interaction))) {
    await replyEphemeral(interaction, '⛔ Hosted server management is restricted to Nexus owners and administrators.');
    return true;
  }
  ...
  if (subcommand === 'list') {
    ...
    if (!response.ok) throw new Error(response.message || 'Hosted-server registry is unavailable.');
    await replyEphemeral(interaction, `🗄️ **Hosted Server Registry**\n\n${privateServerList(response.servers || [])}`);
    return true;
  }

  if (subcommand === 'add') {
    const response = await context.backend.addHostedServer(addInput(interaction));
    if (!response.ok) throw new Error(response.message || 'Unable to register hosted server.');
    await context.refresh?.();
    await replyEphemeral(interaction, `✅ **${response.server.name}** registered as **${response.server.id}**.\n\nThe host and management ports remain private. ${response.server.public === false ? 'This server is hidden from #game-servers.' : 'The public #game-servers registry has been refreshed.'}`);
    return true;
  }

  if (subcommand === 'edit') {
    const id = String(optionValue(interaction, 'id') || '').trim().toUpperCase();
    const response = await context.backend.updateHostedServer(id, editInput(interaction));
    if (!response.ok) throw new Error(response.message || response.code || 'Unable to edit hosted server.');
    await context.refresh?.();
    await replyEphemeral(interaction, `✅ **${response.server.name}** (${response.server.id}) updated and the registry refreshed.`);
    return true;
  }

  if (subcommand === 'remove') {
    const id = String(optionValue(interaction, 'id') || '').trim().toUpperCase();
    if (optionValue(interaction, 'confirm') !== true) {
      await replyEphemeral(interaction, 'Removal cancelled. Run the command again with **confirm: true** to remove the server.');
      return true;
    }
    const response = await context.backend.removeHostedServer(id);
    if (!response.ok) throw new Error(response.message || response.code || 'Unable to remove hosted server.');
    await context.refresh?.();
    await replyEphemeral(interaction, `🗑️ **${id}** removed from the hosted-server registry.`);
    return true;
  }

  if (subcommand === 'refresh') {
    const result = await context.refresh?.();
    await replyEphemeral(interaction, `🔄 #game-servers refreshed${result?.tracked !== undefined ? ` • ${result.tracked} tracked server${result.tracked === 1 ? '' : 's'}` : ''}.`);
    return true;
  }
