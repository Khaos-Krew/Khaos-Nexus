...
function serverChoices(registry = new ArkClusterRegistry()) {
  ...
  try {
    for (const server of registry.list({ includeDisabled: true })) {
      add(server.mapName || server.name || server.envPrefix, server.envPrefix);
    }
  } catch {}
...
async function handleCommand(interaction, config) {
  ...
  if (['configure', 'password', 'send', 'clear'].includes(sub) && !isOwner(interaction, config)) {
    throw new Error('RCON configuration and raw command execution are restricted to the Nexus owner.');
  }
