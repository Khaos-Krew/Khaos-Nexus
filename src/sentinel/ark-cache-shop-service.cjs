...
function orderView(row = {}) {
  ...
  return {
    id: String(row.id || ''),
    publicCacheId: String(row.public_cache_id || ''),
    purchaseNonce: String(row.purchase_nonce || ''),
    discordUserId: String(row.discord_user_id || ''),
    playerEosId: String(row.player_eos_id || ''),
    cacheType: String(row.cache_type || ''),
    pointCost: Number(row.nexus_point_cost || 0),
    species: String(row.species || ''),
    rarity: String(row.rarity || ''),
    variant: String(row.variant || ''),
    blueprint: String(row.blueprint || ''),
    level: Number(row.rolled_level || 0),
    sex: String(row.sex || ''),
...
async function arkShopColumns(connection, database, table) {
  ...
  const ids = ['eosid','eos_id','eos','steamid','steam_id','playerid','player_id']
    .map((key) => byLower.get(key)).filter(Boolean);
...
async function findPointsAccount(connection, dbConfig, eosId, { lock = false } = {}) {
  ...
  for (const idColumn of columns.ids) {
    const [rows] = await connection.execute(
      `SELECT ${safeName(idColumn)} AS player_id, ${safeName(columns.points)} AS points FROM ${table} WHERE ${safeName(idColumn)}=? LIMIT 1${lock ? ' FOR UPDATE' : ''}`,
      [eosId]
    );
...
class ArkCacheShopService {
  constructor({ identityStore = new ArkIdentityStore(), connector = connectMysql, rngSecret = process.env.NEXUS_DINO_CACHE_RNG_SECRET } = {}) {
  ...
  linkedAccount(discordUserId) {
    return pickLinkedArkAccount(this.identityStore.profileByDiscord(cleanId(discordUserId, 25)));
  }
  ...
  async shopper(discordUserId) {
    ...
    try {
      ...
      return { discordUserId: cleanId(discordUserId, 25), account, points: Number(points.row.points || 0) };
  ...
  async purchase({ discordUserId, cacheId, purchaseNonce } = {}) {
    ...
    const nonce = cleanId(purchaseNonce, 80);
    ...
    try {
      ...
      try {
        ...
        if (existing) {
          ...
          return { order: orderView(existing), duplicate: true, balance: null };
        }
        ...
        const identity = `discord-cache:${nonce}:${userId}:${account.eosId}:${type}`;
        const roll = committedRoll(type, this.rngSecret, identity);
        const id = crypto.randomUUID();
        const publicCacheId = `NC-${id.replace(/-/g, '').slice(0, 12).toUpperCase()}`;
        const table = safeName(config.table);
        const debit = await connection.execute(
          `UPDATE ${table} SET ${safeName(wallet.pointsColumn)}=${safeName(wallet.pointsColumn)}-? WHERE ${safeName(wallet.idColumn)}=? AND ${safeName(wallet.pointsColumn)}>=?`,
          [cache.price, account.eosId, cache.price]
        );
        ...
        await connection.execute(`INSERT INTO ${ORDER_TABLE}
          (id, public_cache_id, purchase_nonce, discord_user_id, player_eos_id, cache_type, nexus_point_cost, species, rarity, variant, blueprint, rolled_level, sex, state)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'AWAITING_DELIVERY')`,
          [id, publicCacheId, nonce, userId, account.eosId, type, cache.price, roll.species, roll.rarity, roll.variant, roll.blueprint, roll.level, roll.sex]);
        await connection.execute(`INSERT INTO ${EVENT_TABLE} (order_id, event_type, actor_discord_user_id, details) VALUES (?, 'PURCHASE_ROLLED', ?, ?)`,
          [id, userId, `Discord-first immutable ${type} cache roll committed before reveal animation.`]);
        await connection.commit();
  ...
  async rewards(discordUserId, limit = 8) {
    ...
    try {
      await ensureSchema(connection);
      const [rows] = await connection.query(`SELECT * FROM ${ORDER_TABLE} WHERE discord_user_id=? ORDER BY created_at DESC LIMIT ${safeLimit}`, [userId]);
