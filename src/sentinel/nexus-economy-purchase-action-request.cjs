...
const PURCHASE_OUTBOX_SCHEMA_VERSION = 2;
...
function validateOutboxRecord(record) {
  ...
  if (typeof payload.itemId !== 'string' || !SAFE_ITEM_ID.test(payload.itemId)) return 'invalid-item-id';
  if (!Number.isSafeInteger(payload.quantity) || payload.quantity < 1 || payload.quantity > 25) return 'invalid-quantity';
  if (payload.currency !== 'Nexus Points') return 'invalid-currency';
  ...
  const expectedDigest = digestCanonical(canonicalRecordPayload(record));
...
function freezeActionRequest(record) {
  // Forward only fields covered by the canonical digest. Additional input fields
  // are untrusted and must never become ActionStore execution metadata.
  const payload = Object.freeze(canonicalRecordPayload(record).payload);
