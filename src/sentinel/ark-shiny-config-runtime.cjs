...
function installRuntime({ delayMs = 30000 } = {}) {
  ...
  const timer = setTimeout(() => void run().then((result) => {
    ...
    else console.log(`[Nexus Sentinal] balanced Shiny config COMPLETE: changed=${result.changed} verified=${result.verified} modDetected=${result.modDetected} stagedBeforeMod=${result.stagedBeforeMod} restartRequired=true restartExecuted=false`);
