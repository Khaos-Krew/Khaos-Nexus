...
const STARTUP_TASK_ID = 'arkshop-profile-health';
...
function startArkShopProfileHealthMonitor(client, config, { setTimeoutFn = setTimeout, setIntervalFn = setInterval } = {}) {
...
function installArkShopProfileHealthExtension() {
  ...
  registerStartupTask({
    ...
    priority: 150,
    run(client) {
      startArkShopProfileHealthMonitor(client, config);
    }
  });
