...
function fitBaseToBudget(base, maxCharacters) {
  ...
  // Preserve titles whenever possible; descriptions and footers are safer places to trim.
  shrink('description');
  shrink('footer', 'text');
  shrink('author', 'name');
  shrink('title');
...
function paginateEmbed(embed = {}, options = {}) {
  ...
  // Reserve enough room for at least one maximum-size field when fields exist.
  ...
  return pages.map((page, index) => {
    ...
    // The safe budget leaves margin for the page marker; enforce it anyway.
...
function buildEmbedMessagePages(embeds = [], options = {}) {
  ...
  // Deliberately use one embed per Discord message. Discord's 6000-character cap is
  // aggregate across every embed in a message, so this avoids a second aggregate-overflow path.
