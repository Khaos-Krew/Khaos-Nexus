...
class RoleMenuManager {
  ...
  async reconcile(guild) {
    ...
    const renderDefinitions = definitions
      .filter((definition) => roles.has(definition.moduleId))
      .map((definition) => ({ ...definition, roleName: roles.get(definition.moduleId).name }));
    ...
    for (let index = 0; index < payloads.length; index += 1) {
      ...
      if (!message) {
        ...
        const candidates = recent ? [...recent.values()].filter((candidate) =>
          candidate.author?.id === this.client.user?.id &&
          candidate.embeds?.some?.((embed) => embed.footer?.text === ROLE_MENU_MARKER) &&
          !messages.some((item) => item.id === candidate.id)) : [];
