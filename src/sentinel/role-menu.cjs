...
function accessDefinitionFor(module) {
  ...
  const roleName = String(layout.accessRoleName || `${module.name} Access`).slice(0, 100);
  const roleAliases = [...new Set([module.name, ...(layout.aliases || []), ...(layout.accessRoleAliases || [])]
    .map((value) => String(value || '').trim())
    .filter((value) => value && value !== roleName))];
  return {
    moduleId: module.id,
    label: module.name,
    roleName,
...
function enabledAccessDefinitions(config = {}, moduleEnabled = {}) {
  return MODULES
    .filter((module) => {
      if (typeof moduleEnabled?.[module.id] === 'boolean') return moduleEnabled[module.id];
      return config.modules?.[module.id]?.enabled !== false;
    })
...
class RoleMenuManager {
  ...
  definitions() {
    const admin = this.state?.getAdminSettings?.() || {};
    return enabledAccessDefinitions(this.config, admin.moduleEnabled || {});
  }
  ...
  async ensureAccessRole(guild, definition) {
    ...
    if (!role) {
      ...
      const names = new Set([definition.roleName, ...(definition.roleAliases || [])]);
      role = roles.find((item) => names.has(item.name)) || null;
    }
    if (!role) {
    ...
    this.state.setAccessRole(definition.moduleId, {
      guildId: String(guild.id), roleId: String(role.id), roleName: role.name, updatedAt: new Date().toISOString()
    });
    return role;
  }
  ...
  async reconcile(guild) {
    ...
    const channel = await this.findTextChannel(guild, this.config.discord?.rolesChannelId, DEFAULT_ROLE_CHANNEL_NAMES);
    if (!channel) return { ok: false, skipped: true, roles: roles.size, warnings, reason: 'Roles channel not found. Set discord.rolesChannelId or use a standard roles channel name.' };

    const renderDefinitions = definitions.filter((definition) => roles.has(definition.moduleId)).map((definition) => ({ ...definition, roleName: roles.get(definition.moduleId).name }));
    const payloads = buildRoleMenuPayloads(renderDefinitions);
    ...
    for (let index = 0; index < payloads.length; index += 1) {
      ...
      if (!message) {
        ...
        const candidates = recent ? [...recent.values()].filter((candidate) => candidate.author?.id === this.client.user?.id && candidate.embeds?.some?.((embed) => embed.footer?.text === ROLE_MENU_MARKER) && !messages.some((item) => item.id === candidate.id)) : [];
        message = candidates[0] || null;
      }
    ...
    this.state.setRoleMenu({ guildId: String(guild.id), channelId: String(channel.id), messageIds: messages.map((message) => String(message.id)), updatedAt: new Date().toISOString() });
  ...
  async removeRulesWebsiteLinks(guild) {
    const channel = await this.findTextChannel(guild, this.config.discord?.rulesChannelId, DEFAULT_RULES_CHANNEL_NAMES);
    if (!channel) return { ok: false, skipped: true, edited: 0, deleted: 0, reason: 'Rules channel not found.' };
    ...
    for (const message of recent.values()) {
      ...
      if (!payload) continue;
      if (!payload.content && !payload.components.length && !message.embeds?.length && !message.attachments?.size) {
    ...
    return { ok: true, skipped: false, edited, deleted };
  }
...
module.exports = { ACCESS_BUTTON_PREFIX, ROLE_MENU_MARKER, RoleMenuManager, accessDefinitionFor, enabledAccessDefinitions, parseAccessButton, buildRoleMenuPayloads, stripWebsiteLines, stripWebsiteComponents, websiteCleanupPayload };
