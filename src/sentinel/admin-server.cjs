...
async function body(req) { const chunks = []; let bytes = 0; for await (const chunk of req) { bytes += chunk.length; if (bytes > 64 * 1024) throw new Error('Admin request is too large.'); chunks.push(chunk); } if (!chunks.length) return {}; return JSON.parse(Buffer.concat(chunks).toString('utf8')); }
function safeModuleId(value) { const id = String(value || '').trim().toLowerCase(); return /^[a-z0-9-]{0,60}$/.test(id) ? id : ''; }
function validAdminToken(value) { const token = String(value || ''); return token.length >= 32 && token.length <= 1024 && !/\s/.test(token); }
function publicHealth(status = null) { const discordReady = Boolean(status?.discordReady); const backendReady = Boolean(status?.backend?.ok); return { ok: true, service: 'nexus-sentinal-admin', state: discordReady ? 'ready' : 'starting', discordReady, backendReady }; }
async function hostedProviderStatus(controller) { const store = currentHostedProviderStore(); if (!store) return null; const status = store.status(); const backend = await controller.backend?.modules?.().catch((error) => ({ ok: false, modules: [], message: String(error?.message || error).slice(0, 240) })) || { ok: false, modules: [] }; return { ...status, ok: status.ok !== false && backend.ok !== false, backendModules: backend.modules || [], backendMessage: backend.message || '' }; }
...
function createSentinalAdminServer(options = {}) {
  ...
  const getController = typeof options.getController === 'function' ? options.getController : () => options.controller || null; const logger = options.logger || console; const pairingAllowed = createPairingLimiter(); const arkBackend = options.arkBackend || new ArkBackendControl({ logger });
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (req.method === 'POST' && url.pathname === '/v1/ark/execute') { const input = await body(req); const result = await arkBackend.execute(input, { source: scope === 'forge' ? 'forge-control-plane' : 'sentinal-admin', correlationId: req.headers['x-correlation-id'] }); return json(res, result.ok ? 200 : 502, result); }
      ...
      if (req.method === 'GET' && url.pathname === '/v1/providers/config') { const status = await hostedProviderStatus(controller); return status ? json(res, 200, status) : json(res, 503, { ok: false, code: 'HOSTED_PROVIDER_CONFIG_UNAVAILABLE' }); }
      if (req.method === 'GET' && url.pathname === '/v1/scan') return json(res, 200, await enhancedScan(controller));
      if (req.method === 'POST' && url.pathname === '/v1/config') { const configured = controller.configure(await body(req)); const backend = await controller.backend?.configureModules?.(configured.settings.moduleEnabled || {}).catch((error) => ({ ok: false, message: String(error?.message || error) })); return json(res, backend?.ok === false ? 502 : 200, { ...configured, backend: backend || null }); }
      if (req.method === 'POST' && url.pathname === '/v1/providers/config') { const store = currentHostedProviderStore(); if (!store) return json(res, 503, { ok: false, code: 'HOSTED_PROVIDER_CONFIG_UNAVAILABLE' }); const saved = store.configure(await body(req)); const backend = await controller.backend?.configureProviders?.(saved.modules || {}).catch((error) => ({ ok: false, message: String(error?.message || error).slice(0, 240), modules: [] })); return json(res, backend?.ok === false ? 502 : 200, { ok: backend?.ok !== false, providerConfig: saved, backend: backend || null }); }
      if (req.method === 'POST' && url.pathname === '/v1/providers/validate') { const input = await body(req); const moduleId = safeModuleId(input.moduleId || input.module || ''); const validation = await controller.backend?.validateProviders?.(moduleId).catch((error) => ({ ok: false, message: String(error?.message || error).slice(0, 240), results: [] })); const store = currentHostedProviderStore(); if (store && moduleId && validation) store.recordValidation(moduleId, validation); return json(res, validation?.ok === false ? 502 : 200, validation || { ok: false, code: 'BACKEND_UNAVAILABLE', results: [] }); }
      if (req.method === 'POST' && url.pathname === '/v1/commands/sync') { await controller.syncCommands(); return json(res, 200, await commandStatus(controller)); }
      if (req.method === 'POST' && url.pathname === '/v1/channels/reconcile') { const input = await body(req); return json(res, 200, await controller.reconcileChannels(safeModuleId(input.moduleId))); }
      if (req.method === 'POST' && url.pathname === '/v1/consoles/refresh') { const input = await body(req); return json(res, 200, await controller.refreshConsoles(safeModuleId(input.moduleId))); }
