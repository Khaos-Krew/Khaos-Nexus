...
const { purchasableRanks, rankAuthority, skuToRankMap } = require('../shared/ranks.cjs');
...
function configuredPaidRoleIds(config = {}) {
  return [...new Set(purchasableRanks()
...
async function fetchConfiguredEntitlements(client, config = {}, options = {}) {
  if (rankAuthority(config) !== 'premium-app') {
    return { ok: true, reason: 'server-shop-roles-authoritative', entitlements: [], pages: 0, truncated: false };
  }
  ...
  let pages = 0;
  let truncated = false;
  while (pages < MAX_ENTITLEMENT_PAGES) {
    ...
    if (!after) break;
    if (pages >= MAX_ENTITLEMENT_PAGES) truncated = true;
  }
...
function stalePaidRoleMemberIds(members, config = {}) {
  const paidRoleIds = new Set(configuredPaidRoleIds(config));
  if (!paidRoleIds.size) return [];
  const ids = [];
  for (const member of valuesOf(members)) {
    if (!member || member.user?.bot) continue;
    const current = currentMemberRoleIds(member);
    if (current.some((roleId) => paidRoleIds.has(roleId))) ids.push(String(member.id || member.user?.id || ''));
  }
...
async function reconcileSupporterEntitlements(client, guild, config = {}, options = {}) {
  ...
  if (!fetched.ok) return { ...fetched, users: [], guildEntitlements: [], changed: 0, failures: 0 };
  const grouped = groupUserEntitlements(fetched.entitlements);

  let cachedMembers = options.members || null;
  if (!cachedMembers && options.includeStaleMembers === true && guild?.members?.fetch) {
    cachedMembers = await guild.members.fetch().catch(() => null);
  }
  const userIds = new Set(grouped.users.keys());
  for (const userId of stalePaidRoleMemberIds(cachedMembers, config)) userIds.add(userId);

  const results = [];
  for (const userId of userIds) {
    results.push(await reconcileMemberSupporterRank(guild, userId, grouped.users.get(userId) || [], config));
  }
  return {
    ...fetched,
    users: results,
