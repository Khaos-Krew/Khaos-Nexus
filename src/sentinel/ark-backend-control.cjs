  ...
  async execute(input = {}, context = {}) {
    ...
    try {
      ...
      if (action === 'server.status' || action === 'server.players') {
        ...
        if (!['gus', 'game'].includes(fileKey)) throw new Error('config.plan currently allows only gus or game INI files.');
        const section = clean(input.section, 160);
        const key = clean(input.key, 160);
        if (!section || !key) throw new Error('config.plan requires section and key.');
        const plan = await setIniValue({
        ...
        data = { ...plan, fileKey, section, key, value: String(input.value ?? '').slice(0, 500) };
      }
      ...
      const result = {
        ...
        action,
        correlationId: id,
        server: { id: server.id, name: server.name, mapName: server.mapName, envPrefix: server.envPrefix },
        data,
        llmCalls: 0,
      ...
      const failure = {
        ...
        code: 'ARK_BACKEND_OPERATION_FAILED',
