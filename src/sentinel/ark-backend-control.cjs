...
function configPlanHash({ serverId, fileKey, section, key, value, currentHash }) {
  return sha256(JSON.stringify({
    ...
    fileKey: clean(fileKey, 20).toLowerCase(),
    section: clean(section, 160),
    key: clean(key, 160),
    value: String(value ?? ''),
...
class ArkBackendControl {
  ...
  listServers() {
    return this.registry.list({ includeDisabled: false }).map((server) => ({
      id: server.id,
      name: server.name,
  ...
  audit(entry) {
    try {
      ...
      this.logger.error?.('[ARK Backend Audit]', error);
    }
  }
  ...
  async clusterHealth() {
    ...
    const servers = [];
    for (const record of polled.servers.filter((item) => item.enabled !== false)) {
      let configAccess = { ok: false, files: {}, error: '' };
      if (record.connections?.sftp !== false) {
        try {
          ...
          configAccess = { ok: false, files: {}, error: clean(error?.message || error, 240) };
        }
      }
      servers.push({
        id: record.id,
        ...
        state: record.runtime?.state || 'offline',
        playerCount: Number(record.runtime?.playerCount) || 0,
        latencyMs: record.runtime?.latencyMs ?? null,
  ...
  async planConfig(server, input) {
    ...
    return {
      ...
      currentHash,
      planHash,
      approvalRequired: preview.changed === true,
      applyAction: 'config.apply'
    };
  }
  ...
  async applyConfig(server, input) {
    ...
    if (!crypto.timingSafeEqual(Buffer.from(suppliedPlanHash), Buffer.from(expectedPlanHash))) {
      throw new Error('Config plan is stale or does not match this change. Run config.plan again before applying.');
    }
    const result = await this.setIniValue({ prefix: server.envPrefix, ...change, dryRun: false });
    if (result.restartRequired && typeof this.registry.setRestartRequired === 'function') {
      this.registry.setRestartRequired(server.id, {
        required: true,
        reason: `${change.fileKey}:${change.section}.${change.key} changed`,
        transactionId: clean(input.correlationId, 80)
      });
    }
    return {
      ...result,
  ...
  async execute(input = {}, context = {}) {
    ...
    try {
      let data;
      if (action === 'cluster.health') {
        data = await this.clusterHealth();
      } else {
        ...
        if (action === 'server.status' || action === 'server.players') {
          ...
          const runtime = {
            state: 'online', playerCount: players.length, players,
            lastCheckedAt: new Date().toISOString(), lastOnlineAt: new Date().toISOString(), lastError: ''
          };
          try { this.registry.updateRuntime(server.id, runtime); } catch {}
          data = action === 'server.players'
            ? { players, playerCount: players.length }
            : { state: 'online', playerCount: players.length, maintenance: server.maintenance === true, restartRequired: server.restartRequired === true };
          ...
          data = await this.applyConfig(server, { ...input, correlationId: id });
        }
      }

      const result = {
        ok: true, action, correlationId: id,
        server: server ? { id: server.id, name: server.name, mapName: server.mapName, envPrefix: server.envPrefix } : null,
        data, llmCalls: 0, durationMs: Date.now() - started, completedAt: new Date().toISOString()
      };
      ...
      const failure = {
        ok: false, action, correlationId: id,
        server: server ? { id: server.id, name: server.name, mapName: server.mapName, envPrefix: server.envPrefix } : null,
        code: 'ARK_BACKEND_OPERATION_FAILED', message: clean(error?.message || error, 300), llmCalls: 0,
