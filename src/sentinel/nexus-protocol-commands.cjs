...
function commandDefinitions() {
  return Object.freeze([
    Object.freeze({
      ...
      description: 'View Nexus Protocol network status and your participation progress.',
      dmPermission: false,
      options: [
        Object.freeze({ type: 'subcommand', name: SUBCOMMANDS.STATUS, description: 'View active Protocol network status.' }),
        Object.freeze({
          type: 'subcommand',
          name: SUBCOMMANDS.PROGRESS,
          description: 'View your account-bound Protocol participation progress.'
        })
      ]
    }),
    Object.freeze({
...
function normalizeCommandIntent(input = {}) {
  ...
  if (command === COMMANDS.PROTOCOL) {
    if (!subcommand || subcommand === SUBCOMMANDS.STATUS) {
      return Object.freeze({ kind: 'read', target: 'protocol_status', accountId: accountId || null });
    }
    if (subcommand === SUBCOMMANDS.PROGRESS) {
      if (!accountId) throw new Error('Protocol progress requires a linked account');
      return Object.freeze({
        kind: 'read',
    ...
    throw new Error('Invalid protocol subcommand');
  }
