...
function detailPayload(cacheId,shopper=null){
  ...
  return {embeds:[{title:`${m.emoji} ${m.name}`,description:m.tagline,color:0xb00020,fields:fields.slice(0,25),image:{url:`attachment://${imageName}`},footer:{text:'Nexus Cache Terminal • Species → Variant → Level → Sex'}}],components:[catalogSelect(cacheId),navigationRow({cacheId,canBuy})],files:[cacheImageAttachment(cacheId)],allowedMentions:{parse:[]}};
}
    ...
    if(!client[BOUND]){
      ...
      client.on(Events.InteractionCreate,(interaction)=>{
        ...
        void(async()=>{
          ...
          if(isSelect){await interaction.deferUpdate();const cacheId=String(interaction.values?.[0]||'').toLowerCase();let shopper=null;try{shopper=await service.shopper(userId);}catch{}return interaction.editReply(detailPayload(cacheId,shopper));}
