...
## Tests
Latest local full suite after integrating #583: 1,411 passed, 0 failed, 0 skipped. npm run check passed.

## Exact remaining work (priority)
A. Database-managed AES-256-GCM RCON provider implemented; map-bound ciphertext, audited writes gate, no DB fallback. Schema script scripts/migrate-rcon-database.cjs prepared, NOT applied.
B. Shared ArkRconClient dynamically resolves Postgres on every command when selected. Migrated explicit construction sites to preserve server prefix. Legacy backend stays selected until approved cutover.
C. /ark server test/status/configure/password/send/clear implemented with existing owner/staff checks; registered even if main ARK context unavailable.
...
## Status
RCON: DB-managed replacement and shared provider implemented/tested; migration and live test not applied. Requires NEXUS_RCON_CONFIG_BACKEND=postgres, NEXUS_RCON_DATABASE_URL, NEXUS_RCON_CONFIG_SECRET, NEXUS_RCON_CONFIG_WRITES_ENABLED.
