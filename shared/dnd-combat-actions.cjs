...
function resolveAttack(state, input = {}, rng = Math.random) {
  ...
  const attack = runtime.resolveAbilityCheck({modifier: core.cleanNumber(input.attackModifier,0,-50,100),dc: target.armorClass,advantage: input.advantage===true&&!cancelAdvantage,disadvantage: hasDisadvantage&&!cancelAdvantage},rng);
  ...
  if (attack.success) {
    ...
    damage = runtime.resolveDamage({count,sides: core.cleanNumber(input.damageDiceSides,6,2,100),modifier: core.cleanNumber(input.damageModifier,0,-100,100),damageType: input.damageType},rng);
    ...
    if (target.currentHp === 0) {
      if (target.characterId) {
        ...
        runtime.appendStateEvent(state, {campaignId:combat.campaignId,runId:combat.runId,sceneId:combat.sceneId,type:'character.condition.applied',actorType:'rules_engine',actorId:'combat',idempotencyKey:`${key}:unconscious`,payload:{characterId:target.characterId,condition:'unconscious'}});
...
function useCombatAction(state, input = {}) {
  core.ensureSoloCombatState(state); runtime.assertOwnerPreview(state); const combat=core.combatById(state,input.combatId); const key=runtime.clean(input.idempotencyKey,160)||runtime.makeId('action'); const duplicate=combat?.log.find((item)=>item.idempotencyKey===key); if(duplicate)return runtime.clone(duplicate.payload); const actor=core.assertActiveActor(combat,input.actorId); const action=ACTIONS.includes(input.action)?input.action:''; if(!action||action==='attack'||action==='cast_spell')runtime.fail('Use the dedicated attack or spell resolver for this action.','DND_COMBAT_ACTION_INVALID'); if(!actor.resources.action)runtime.fail('The combatant has already used its action.','DND_COMBAT_ACTION_SPENT'); actor.resources.action=false; if(action==='dash')actor.resources.movement*=2; if(action==='dodge'&&!actor.conditions.includes('dodging'))actor.conditions.push('dodging'); if(action==='disengage'&&!actor.conditions.includes('disengaging'))actor.conditions.push('disengaging'); actor.updatedAt=runtime.nowIso(); const result={actorId:actor.id,action,targetId:runtime.clean(input.targetId,100)}; core.appendCombatLog(combat,{idempotencyKey:key,type:'action',payload:result}); return runtime.clone(result);
}
...
function castSpell(state, input = {}) {
  ...
  if (level > 0) {
    ...
    if (actor.characterId) runtime.appendStateEvent(state, {campaignId:combat.campaignId,runId:combat.runId,sceneId:combat.sceneId,type:'character.spell_slots.changed',actorType:'rules_engine',actorId:'combat',idempotencyKey:`${key}:slot:${level}`,payload:{characterId:actor.characterId,level,value:actor.spellSlots[level]}});
  }
  actor.resources[actionResource] = false;
  if (input.concentration === true) actor.concentration = runtime.clean(input.spellName, 200);
  const result = {actorId:actor.id,spellName:runtime.clean(input.spellName,200),level,concentration:actor.concentration,targetIds:(input.targetIds||[]).map((item)=>runtime.clean(item,100)).filter(Boolean)};
